...
def parse_result_page(html, horse_token, horse_no):
    ...
    if corners:
        ...
        out["cornerRaw"] = ""
    tr = find_result_row(html, horse_token)
    if tr:
        vals = [strip_tags(c) for c in re.findall(r"<t[dh]\b[^>]*>(.*?)</t[dh]>", tr, re.I | re.S)]
...
def main():
    ...
    for i, info in enumerate(links, 1):
        ...
        try:
            hn = extract_horse_number(race_html, tok)
            ...
            for run in runs:
                ...
                try:
                    ...
                    rh = cache[ru]
                    rn = extract_horse_number(rh, tok)
                    run["horse_number"] = rn
                    extra = parse_result_page(rh, tok, rn)
