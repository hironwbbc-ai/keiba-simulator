...
function learningBeforeRace(targetRace){
  ...
  const usable=source.records
    .filter(r=>r?.deltaAfter && `${historyDateKey(r.date||"")}|${String(r.venue||"").padStart(2,"0")}|${String(Number(r.no)||999).padStart(2,"0")}` < target)
    .sort((a,b)=>`${historyDateKey(a.date||"")}|${String(a.venue||"").padStart(2,"0")}|${String(Number(a.no)||999).padStart(2,"0")}`.localeCompare(`${historyDateKey(b.date||"")}|${String(b.venue||"").padStart(2,"0")}|${String(Number(b.no)||999).padStart(2,"0")}`));
...
function learningOrderKey(histRace){
  ...
  const v=histRace?.venue||codeToVenue(histRace?.venue_code)||"";
  const n=Number(histRace?.no??histRace?.race_number);
  return `${d}|${String(v).padStart(2,"0")}|${String(Number.isFinite(n)?n:999).padStart(2,"0")}`;
}
...
function trainFromBacktest(histRace,result,learningInput=null){
  ...
  const latest=stored.records
    .filter(r=>r?.key)
    .map(r=>`${historyDateKey(r.date||"")}|${String(r.venue||"").padStart(2,"0")}|${String(Number(r.no)||999).padStart(2,"0")}`)
    .sort().pop();
